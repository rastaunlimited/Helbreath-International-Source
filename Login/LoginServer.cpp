    ...
    if (pFile == NULL) {
		...
		while( token != NULL )
                    ...
                    if (IsSame(token, "login-server-port"))
						 ...
						 else if (IsSame(token, "server-dns"))
                         {
                          token = pStrTok->pGet();
                          SafeCopy(m_cGameServerDns, token);
                          sprintf(LogBuff, "(*) server dns : %s", m_cGameServerDns);
						  PutLogList(LogBuff);
                         }
		...
		if (pFile != NULL) fclose(pFile);
				char szHostName[255];
					...
					GetIPByDNS();
					wsprintf(cTxt, "(*) Login address: %s External Address: %s Gate server port: %d",ListenAddress, m_cGameServerAddrExternal, GateServerPort);
					PutLogList(cTxt);
...
void CLoginServer::ProcessGSMsgWithDBAccess(char *pData, BYTE GSID, MYSQL myConn)
{
 DWORD *dwpMsgID;
	...
	case MSGID_REQUEST_NOSAVELOGOUT:
		ProcessClientLogout(pData+6, FALSE, 0, (BYTE)GSID, myConn);
		break;

	case MSGID_REQUEST_SAVEPLAYERDATALOGOUT:
		ProcessClientLogout(pData+6, TRUE, 1, (BYTE)GSID, myConn);
		break;

	case MSGID_REQUEST_SAVEPLAYERDATA_REPLY:
		ProcessClientLogout(pData+6, TRUE, 2, (BYTE)GSID, myConn);
		break;
...
void CLoginServer::SendCharList(char* AccountName, WORD ClientID, MYSQL myConn)
{
 ...
 char Txt100[100], Txt500[500], GoodAccName[25], QueryConsult[200], Txt[250];
			  ...
			  PutOffsetValue(Txt500, 11, BYTESIZE, Valid);
...
bool CLoginServer::IsAccountValid(char *AccountName, WORD ClientID, MYSQL myConn)
{
	...
	{
		ZeroMemory(Txt, sizeof(Txt));
		sprintf(Txt, "(!) Account [%s] Is Not Valid, Intial 30 Days For Account Vaild!", GoodAccName);
...
void CLoginServer::GetIPByDNS()
{
	char cDnsResult[40], cTxt[120];
	struct hostent *host_entry;

	ZeroMemory(m_cGameServerAddrExternal, sizeof(m_cGameServerAddrExternal));
	if (m_cGameServerDns[0] >= 65 && m_cGameServerDns[0] <= 122) {

		host_entry = gethostbyname(m_cGameServerDns);
		if (host_entry == NULL){
			PutLogList("Failed to get DNS entry!");
			return;
		}
		...
		strcpy(m_cGameServerAddrExternal, cDnsResult);
	}
	else strcpy(m_cGameServerAddrExternal, m_cGameServerDns);
	wsprintf(cTxt,"(!!) Game Server External Address %s", m_cGameServerAddrExternal);
	PutLogList(cTxt);
}